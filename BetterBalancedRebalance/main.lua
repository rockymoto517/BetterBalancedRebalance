...
if not BetterRebalanceTable then
	...
	function BetterRebalanceTable:load_localization()
		Hooks:Add(
			...
			"LocalizationManagerPostInit_BetterRebalanceTable", -- Zamn that's a long id
			function(loc)
				loc:load_localization_file(
					BetterRebalanceTable.mod_path .. "Loc/en.json"
				)
			end
		)
	end
...
-- Thanks hoppip
if
	RequiredScript and not BetterRebalanceTable.loaded_scripts[Requiredscript]
then
