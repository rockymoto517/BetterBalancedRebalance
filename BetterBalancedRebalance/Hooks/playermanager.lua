...
function PlayerManager:body_armor_skill_addend(override_armor)
	...
	addend = addend + self:upgrade_value("player", tostring(override_armor or managers.blackmarket:equipped_armor(true, true)) .. "_crook_armor_addend", 0)
	return addend
end
...
function PlayerManager:increment_gambler_pool()
	...
	if self._gambler_pool >= 20 then
		...
		if self:has_category_upgrade("player", "gambler_kill_to_ammo_team") then
			managers.network:session():send_to_peers_synched("sync_unit_event_id_16", self:local_player(), "inventory", 10)
...
function PlayerManager:body_armor_regen_multiplier(moving, health_ratio)
	local passive_zerk = managers.player:upgrade_value("player", "passive_always_zerk", false)
	local multiplier = old_body_armor_regen_mul(self, moving, not passive_zerk and health_ratio)

	if health_ratio and passive_zerk then
		multiplier = multiplier * (1 - managers.player:upgrade_value("player", "armor_regen_damage_health_ratio_multiplier", 0))
	end
	return multiplier
end
...
function PlayerManager:movement_speed_multiplier(speed_state, bonus_multiplier, upgrade_level, health_ratio)
	...
	local multiplier = old_move_speed_mul(self, speed_state, bonus_multiplier, upgrade_level, not passive_zerk and health_ratio)
	if health_ratio and passive_zerk then
		multiplier = multiplier * (1 + managers.player:upgrade_value("player", "movement_speed_damage_health_ratio_multiplier", 0))
	end
	return multiplier
end

-- Infiltrator DR on melee hit
local old_dr_mul = PlayerManager.damage_reduction_skill_multiplier
function PlayerManager:damage_reduction_skill_multiplier(...)
	local multiplier = old_dr_mul(self, ...)
	if managers.player:temporary_upgrade_active("temporary", "melee_damage_reduction") then
		multiplier = multiplier * managers.player:temporary_upgrade_value("temporary", "melee_damage_reduction", 1)
	end

	return multiplier
end

-- Custom cooldown thingy to allow for different cooldowns and active times
function PlayerManager:activate_temporary_upgrade(category, upgrade)
	...
	local active_time = upgrade_value[3]
	...
	-- New active time for cooldown based upgrades
	if active_time then
		self._temporary_upgrades[category][upgrade].active_time = Application:time() + active_time
	end
...
-- Check if a cooldown upgrade is active
function PlayerManager:temporary_upgrade_active(category, upgrade)
	local upgrade_value = self:upgrade_value(category, upgrade)
	...
	-- Custom active times
	if self._temporary_upgrades[category][upgrade].active_time then
		return self._temporary_upgrades[category][upgrade].active_time > Application:time()
	end
