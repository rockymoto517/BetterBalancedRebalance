BetterRebalanceTable:PostHook(NewRaycastWeaponBase, "_update_stats_values", function(self)
	-- Make trigger happy give pistols shield piercing
	if managers.player:has_category_upgrade("pistol", "pierce_enemy_armor") then
		if self:_check_weapon_category("pistol") and not self._can_shoot_through_enemy and self._armor_piercing_chance ~= 1 then
